...
public class TcpConnection implements Runnable {
     ...
     */
    public void sendPhotoName(final String photo) {
        ThreadUtil.executeOnNewThread(new Runnable() {
            @Override
            public void run() {
                if (socketOut != null) {
                    socketOut.println(photo);
                    socketOut.flush();
                }
            }
        });
     ...
     */
    public void sendMemoryStatus(final Long available, final Long needed) {
        ThreadUtil.executeOnNewThread(new Runnable() {
            double _available = available.doubleValue();
            double _needed = needed.doubleValue();
            double memory = Math.round((_available / needed) * 100);
            @Override
            public void run() {
                if (socketOut != null) {
                    socketOut.println("AVAILABLE: " + Long.toString(available));
                    socketOut.println("NEEDED: " + Long.toString(needed));
                    socketOut.println("MEMORY: " + Double.toString(memory)+ "%");
